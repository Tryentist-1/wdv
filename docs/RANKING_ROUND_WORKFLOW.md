# Feature Brief: Redesigned Ranking Round Workflow (R300)
...
graph TD
    subgraph "Revised Ranking Round App Flow (Bale-Centric)"
        ...
        H1 --> I1["Export Modal (Verify, Screenshot, JSON, Email)"];
        I1 --> D;
    end
...
2.  **The Scoring View (The Digital Clipboard):**
    ...
    *   After scoring the end for all archers, they tap a "Next End" button, which advances the round (10 ends total for R300).
...
4.  **Verification and Export (Unified Export Modal):**
    *   The individual scorecard view exposes a single "Export" action.
    *   The Export modal provides: Verify & Send (bale totals SMS), Screenshot, Download JSON, Copy JSON, and Email Coach.
	* 	Results may be sent via SMS using the device's messaging intent.
	* 	Optionally, JSON can be downloaded or copied for server-side ingestion.
	
### 5. Notes and Next Steps

R300 (10x3) is now the default season flow. The classic R360 remains available. Next, consider adding a setup-time round selector to toggle between 300 and 360 in-app without switching pages. 